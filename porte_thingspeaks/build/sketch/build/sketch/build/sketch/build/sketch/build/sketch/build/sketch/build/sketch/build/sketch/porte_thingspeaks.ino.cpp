#include <Arduino.h>
#line 1 "C:\\Users\\cgueble\\Documents\\Arduino\\21-Wifi-porte_thingspeaks-bis\\porte_thingspeaks\\porte_thingspeaks.ino"
#line 1 "C:\\Users\\cgueble\\Documents\\Arduino\\21-Wifi-porte_thingspeaks-bis\\porte_thingspeaks\\porte_thingspeaks.ino"
...
const int InterphoneSonette = 0; //l'acquisition de la sonette de l'interphone se fera sur GPIO2 c'est a dire D2 c'est a dire la pin 3 de la carte ESP8266-E01 ; c'est une INPUT
...
const int InterphonePorte = 1; //la commande de l'interphone se fera sur GPIO1 c'est a dire D7 c'est a dire la pin 2 de la carte ESP8266-E01 ; c'est une OUTPUT
...
boolean Status_InterphoneSonette = false;
boolean Status_Ouverture_authorise = false;
boolean TraitementVerrouEnCours = false;                 // true pour iniber la fonction verrou
boolean TraitementInterphoneSonetteEnCours = true;      // true pour iniber la fonction verrou
int MaxDebounse = 8;
...
void setup() {
  ...
  //delay(5000);//Attente 5s avant de debuter setup
  //Set up PIN in INPUT
  pinMode(InterphoneSonette, INPUT_PULLUP); //LOW : Sonette silentieuse
  pinMode(IlsPorte, INPUT_PULLUP);
  pinMode(Verrou, INPUT_PULLUP);

  //Set up PIN in OUTPUT
  //pinMode(InterphonePorte, OUTPUT);
  //digitalWrite(InterphonePorte, LOW);//CGU test

  if (SERIAL_PORT_LOG_ENABLE) {
    Serial.println(F("InterphoneSonette= 0 (GPIO2), INPUT_PULLUP"));
    Serial.println(F("IlsPorte= 2 (GPIO0), INPUT_PULLUP"));
...
void loop() {
  ...
  Read_Button(IlsPorte, Verrou, InterphoneSonette); // lecture du statut des contacts

  // si un changement de status est d̩tect̩, on essaye de mettre a jour le statut thingspeak
  ...
  if (Status_Verrou != Old_Status_Verrou )
  {
    ...
    while ((Cpt_Verrou <= 120) && (Status_IlsPorte == true)) { //??? Status_IlsPorte == true ou False ?????
      delay(1000);
      Read_Button(IlsPorte, Verrou, InterphoneSonette);
  ...
  if (Status_InterphoneSonette == true && TraitementInterphoneSonetteEnCours == false)
  {
    TraitementInterphoneSonetteEnCours = true;
    if (SERIAL_PORT_LOG_ENABLE) {
      Serial.println("Debut traitement InterphoneSonette");
    }
    //Code Laurent a placer ici
    TraitementInterphoneSonetteEnCours = false;
  }//fin Status_InterphoneSonette==true

  //debut daily mail

  if (Hour == 8) // tant qu'il est 8h locale, on r̩ autorise le daily mail
  {
    DailyMailSent = false;
  ...
  if ((Hour > 8) && (DailyMailSent == false)) // a partir de 9h00 locale, si le Daily mail n'a pas ̩t̩ envoy̩, alors on l'envoie
  {
    mqttpublishtry();
    GetTimeByUDP();
    Read_Button(IlsPorte, Verrou, InterphoneSonette);
    ...
    else // Mail sent failure
    {
      ...
      delay(3000); // on bloque l'envoie du Daily mai pour 3 minutes avant prochain envoie
    }
  }//FIN Daily mail
...
void Read_Button(boolean IlsPorte, boolean Verrou, boolean InterphoneSonette)
{
  int DebounseCpt = 0;
  ...
  int Status_InterphoneSonette_Cpt = 0;
  int Threshold = 4;
  while (DebounseCpt <= MaxDebounse) {
    DebounseCpt = DebounseCpt + 1;
    if (digitalRead(IlsPorte) == HIGH) Status_IlsPorte_Cpt = Status_IlsPorte_Cpt + 1; // ATTENTION - pour application reel : HIGH (porte ouverte -> ILS ouvert -> HIGH level)
    if (digitalRead(Verrou) == HIGH) Status_Verrou_Cpt = Status_Verrou_Cpt + 1; // ATTENTION - pour application reel : HIGH
    if (digitalRead(InterphoneSonette) == HIGH) Status_InterphoneSonette_Cpt = Status_InterphoneSonette_Cpt + 1; // ATTENTION - pour application reel : HIGH
    delay(100);
  }

  if (Status_IlsPorte_Cpt > Threshold) Status_IlsPorte = true; // ATTENTION - pour application reel : HIGH (porte ouverte -> ILS ouvert -> HIGH level)
  else Status_IlsPorte = false;
  if (Status_Verrou_Cpt > Threshold ) Status_Verrou = true; // ATTENTION - pour application reel : HIGH
  else Status_Verrou = false;
  if (Status_InterphoneSonette_Cpt > Threshold ) Status_InterphoneSonette = true; // ATTENTION - pour application reel : HIGH
  else Status_InterphoneSonette = false;
  if (SERIAL_PORT_LOG_ENABLE) {
      ...
      Serial.print("Status_InterphoneSonette_Cpt = ");
      Serial.println(Status_InterphoneSonette_Cpt);      
    }
...
byte sendEmail(String FcMailFrom, String FcMailTo, String FcMailContent, String FcThingspeakChannelAdress, String FcThingspeakChannelWriteAPIKey)
{
  ...
  client.print("SW version: 21/01/2018  ;  "); // Date de compilation
...
void mqttpublish() {
  ...
  mqttClient.publish(PublishCmd, msgBuffer);
